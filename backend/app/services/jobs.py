...
try:
    ...
    logger.info("Celery available for background task processing")
except ImportError:
...
def _analyze_reference_performance_sync(perf_id: int) -> Dict[str, Any]:
    """
    Synchronous implementation of reference performance analysis.
    """
    db = SessionLocal()
...
def _analyze_student_performance_sync(perf_id: int) -> Dict[str, Any]:
    """
    Synchronous implementation of student performance analysis.
    """
    db = SessionLocal()
...
if CELERY_AVAILABLE:
    ...
    def analyze_reference_performance_task(self, perf_id: int) -> Dict[str, Any]:
        """
        Analyze a reference performance in the background.
        """
        self.update_state(state='PROGRESS', meta={'step': 'Loading performance'})
        return _analyze_reference_performance_sync(perf_id)

    @celery_app.task(bind=True, name='analyze_student_performance')
    def analyze_student_performance_task(self, perf_id: int) -> Dict[str, Any]:
        """
        Analyze a student performance in the background.
        """
        self.update_state(state='PROGRESS', meta={'step': 'Loading performance'})
        return _analyze_student_performance_sync(perf_id)

    @celery_app.task(bind=True, name='process_audio_file')
    def process_audio_file_task(self, audio_data: bytes, filename: str, score_id: int, role: str, source: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                if role == 'reference':
                    analysis_task = analyze_reference_performance_task.delay(performance.id)
                else:
                    analysis_task = analyze_student_performance_task.delay(performance.id)
...
# Public API functions that work with or without Celery
def analyze_reference_performance(perf_id: int) -> Dict[str, Any]:
    """
    Analyze a reference performance.
    Uses Celery if available, otherwise runs synchronously.
    """
    if CELERY_AVAILABLE:
        task = analyze_reference_performance_task.delay(perf_id)
        return {'task_id': task.id, 'status': 'queued'}
    else:
        return _analyze_reference_performance_sync(perf_id)


def analyze_student_performance(perf_id: int) -> Dict[str, Any]:
    """
    Analyze a student performance.
    Uses Celery if available, otherwise runs synchronously.
    """
    if CELERY_AVAILABLE:
        task = analyze_student_performance_task.delay(perf_id)
        return {'task_id': task.id, 'status': 'queued'}
    else:
        return _analyze_student_performance_sync(perf_id)


def process_audio_file(audio_data: bytes, filename: str, score_id: int, role: str, source: str) -> Dict[str, Any]:
    """
    Process uploaded audio file.
    Uses Celery if available, otherwise runs synchronously.
    """
    if CELERY_AVAILABLE:
        task = process_audio_file_task.delay(audio_data, filename, score_id, role, source)
        return {'task_id': task.id, 'status': 'queued'}
    else:
        ...
        try:
            ...
            logger.info(f"Created performance {performance.id}")
            
            # Run analysis synchronously
            if role == 'reference':
                analysis_result = _analyze_reference_performance_sync(performance.id)
            else:
                analysis_result = _analyze_student_performance_sync(performance.id)
...
def get_task_status(task_id: str) -> Dict[str, Any]:
    ...
    Returns error if Celery is not available.
    """
    if not CELERY_AVAILABLE:
        return {
            'task_id': task_id,
            'state': 'NOT_AVAILABLE',
            'error': 'Celery is not available - tasks run synchronously',
